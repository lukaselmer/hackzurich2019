...
type P = {
  items: { emphasized?: boolean; timespan: string; price: string; description: string; id: string }[];
  onChoosePlan: (arg0: any) => void;
    ...
    <div className="columns is-centered is-variable is-5">
      {props.items.map((item, index) => (
        <div className="Pricing__column column" key={index}>
          <div className={'Pricing__card card' + (item.emphasized === true ? ' emphasized' : '')}>
            <div className="Pricing__card-content card-content">
              <div className="Pricing__period has-text-weight-bold">{item.timespan}</div>
